...
const AMAP_KEY = 'e64fe861da803b22ac6968de8c8c8c5d';
...
app.get('/api/geocode', async (req, res) => {
  try {
    ...
    const url = `https://restapi.amap.com/v3/place/text?key=${AMAP_KEY}&keywords=${encodedAddress}&city=&offset=1&output=json`;
    ...
    if (result && result.pois && result.pois.length > 0) {
      res.json({
        lat: parseFloat(result.pois[0].location.split(',')[1]),
        lng: parseFloat(result.pois[0].location.split(',')[0]),
      });
    } else {
